...
name: draft-to-blog
description: |
  Use when converting reading notes, rough drafts, or draft directories from `./draft/` into polished Nuxt Content blog articles in `./content/blog/` for this personal site. Trigger on requests to publish notes, turn drafts into posts, process reading notes, convert drafts, or move material from `draft/` to the blog.
---
...
Before writing, use `rg --files content/blog` to find posts, then read at least 2-3 existing blog posts from `./content/blog/` to internalize:
...
- Use Codex's available web/search capability to verify from credible sources (official docs, papers, reputable engineering blogs). If internet access is unavailable, say what could not be verified instead of guessing.
...
Before editing, briefly tell the user what files you plan to create or modify. Write each article as a markdown file in `./content/blog/`. Follow these rules strictly:

**Frontmatter:**
```yaml
---
title: 文章标题
date: YYYY-MM-DD  # use the current date from the active environment
category: 分类名称
tags: [标签1, 标签2]
---
```
...
- Whether build or content validation was run, and the result
